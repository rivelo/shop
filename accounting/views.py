...
def client_result(request, tdelta = 30, id = None, email=False):
    ...
    b_bike = Bicycle_Sale.objects.filter(client_id=user_id).values(
        'model__model__model', 'model__model__brand__name', 'model__serial_number', 'model__size__name', 'model__size__letter', 'date', 'service', 'id'
    )
...
def shopdailysales_add(request, id=None):
    ...
    if id is not None:
        ...
        shopN = Shop.objects.filter(id=shop_id_from_ip.id).first()
        if not shopN:
